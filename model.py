...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer.wte.weight = (
            self.lm_head.weight
        )  # Ties embedding and output weights
...
class ProductGPT(GPT):
    def __init__(self, config):
        ...
        self.product_embedding = nn.Embedding(
            config.max_products + 1,  # +1 for padding
            config.n_product_features,
    ...
    def forward(
        ...
        """
        Args:
            context_features: Tensor(batch_size, n_context_features) - Always available session context
            user_id: Optional[Tensor(batch_size)] - User IDs if known
            user_metadata: Optional[Tensor(batch_size, n_user_metadata)] - User features if available
            product_history: Optional[Tensor(batch_size, seq_len)] - Previous interactions if any
            targets: Optional[Tensor] - Target products for training
        """
        device = context_features.device
        batch_size, seq_len, _ = (
            context_features.size()
        )  # Now context_features is [batch, seq, features]
        ...
        # 4. Process product history (if available)
        if product_history is not None:
            prod_emb = self.transformer.wte(product_history)  # [batch, seq, n_embd]
            pos = torch.arange(0, seq_len, dtype=torch.long, device=device)
            pos_emb = self.transformer.wpe(pos)  # [seq, n_embd]
        else:
            prod_emb = torch.zeros(
                batch_size, seq_len, self.config.n_embd, device=device
            )
